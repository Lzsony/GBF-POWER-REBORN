...
def verify_version(path):
    ...
    for high, low in [(fixed[2], fixed[3]), (fixed[4], fixed[5])]:
        assert (high >> 16, high & 0xffff, low >> 16, low & 0xffff) == (0, 1, 0, 0), 'Unexpected executable version'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='.package-', dir=OUTPUT) as temporary:
        ...
        output = OUTPUT / 'GBF-POWER-REBORN-0.1.0-windows-x64.zip'
